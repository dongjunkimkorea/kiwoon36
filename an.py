...
if __name__ == "__main__":
    ...
    collect = pd.DataFrame([], columns =['종목코드','일자','현재가','개인누적합계'
                           ,'개인최고저점','개인매집수량','개인매집고점','개인분산비율'
    ...
    con = sqlite3.connect("c:/db/kosdap.db")
    collect.to_sql(analyze_db_name, con, if_exists='replace')
...
#    plt.plot(x, y, color='r', label='M1')
#    #plt.plot(dfAn.index, collect['외국인분산비율'], color='g', label='M1')
#    #plt.plot(dfAn.index, collect['금융투자분산비율'], color='b', label='M1')
#    #
#    plt.show()
